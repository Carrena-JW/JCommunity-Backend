...
public class SetPostCommentLike
{
    public record Command : ICommand<Command, bool>
    {
        ...
        internal class Handler : IRequestHandler<Command, Result<bool>>
        {
            ...
            public async Task<Result<bool>> Handle(Command command, CancellationToken token)
            {
                ...
                if (postComment == null)
                {
                    return Result.Fail(new PostError.CommentNotFound(postCommentId.ToString()));
                }
                ...
                _logger.LogInformation("Set Post Comment Like - like: {@like}", like);
                return true;
            }
        }
        #endregion
